...
test('Create a new book', async ({ page, request }) => {
  ...
  await page.context().addCookies([{ name: 'token', value: authToken, domain: 'swe.flaig.io', path: '/', httpOnly: false, secure: true }]);
} catch (error) {
  ...
  await page.getByLabel('Untertitel', { exact: true }).fill('Ein Buch erstellt durch einen E2E Test');
