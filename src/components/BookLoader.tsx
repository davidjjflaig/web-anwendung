...
export default function BooksPage() {
  ...
  const ladeDaten = async (pageTarget: number = 1) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Server-Fehler';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <div className="card bg-base-100 shadow-lg p-6 border border-base-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <button onClick={() => ladeDaten(1)} className="btn btn-primary">Suchen</button>
        </div>
      </div>
            ...
            {data.content.map(buch => (
              <div key={buch.id} className="card bg-base-100 shadow-md border border-base-200">
                <div className="card-body">
                  <h2 className="card-title">{buch.titel?.titel || 'Unbekannter Titel'}</h2>
                  <p className="text-sm opacity-60">{buch.titel?.untertitel !== 'null' ? buch.titel?.untertitel : ''}</p>
                  <div className="card-actions justify-end mt-4">
                    <button className="btn btn-sm btn-outline" onClick={() => navigate(`/buecher/${buch.id}`)}>Details</button>
                  </div>
                </div>
              </div>
            ))}
